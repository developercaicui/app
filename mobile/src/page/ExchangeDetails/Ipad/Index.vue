		...
		<div id="content1">
          ...
          <dd>
            <div class="name"><span>{{ data.nikeName }}</span>
              <div @click="addAnswer(this)" isadd="yes" class="add-answer" v-if='data.memberId == JSON.parse(this.webApi.getCookie("userInfo")).memberId'><i>...</i><span>问题补充</span></div>
            </div>
  ...
  methods: {
        ...
        setListData(list) {
        	this.taskprogress = list.taskprogress;
          	...
          	list.taskprogress = `${list.taskprogress != '-1' && list.taskType != ' ' && list.taskType == 'video' && list.courseId && list.courseId != ' ' && list.chapterId && list.chapterId != ' ' && list.taskId && list.taskId != ' '?this.webApi.formatType(list.taskType,list.taskprogress):''}`;
        },
        setReplysData(list) {
          list.map(item =>{
